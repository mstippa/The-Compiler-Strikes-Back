...
function parseVarDecl2() {
		...
		if (scopeCheck > -1 && identifiers[scopeCheck+1] === scope) {
			semanticAnalysisError[parse2ErrorCounter] = [currentTokenValue + " was already declared in scope " + scope];
		}
...
function parseAssignmentStatement2() {
	if (assignStatementCount > varDeclCount) {
		semanticAnalysisError[parse2ErrorCounter] = [currentTokenValue + " was not declared"];
		parse2ErrorCounter++;
	}	
...
function displayParse2Outcome() {
	if (semanticAnalysisError.length === 0) {
